    ...
    if card_ids := browser.selected_cards():
        return (
            CollectionOp(
                ...
                parent=browser,
                op=lambda col: (bury_cards(col, card_ids) if checked else unbury_cards(col, card_ids)),
            )
            .success(lambda out: tooltip(f"{len(card_ids)} cards buried.", parent=browser) if out else None)
            .run_in_background()
        )
...
def update_toggle_bury_action(browser: Browser) -> None:
    if hasattr(browser, ATTR_NAME):
        is_current_buried = bool(browser.current_card and is_buried(browser.current_card))
