...
def act_on_card(col: Collection, card: Card) -> ResultWithChanges:
    pos = col.add_custom_undo_entry(f"Mortician: {action_description()} difficult card")
...
def on_did_answer_card(reviewer: Reviewer, card: Card, ease: Literal[1, 2, 3, 4]) -> None:
    ...
    if agains >= threshold(card):
        if config["tag"] or config["flag"] or config.action != Action.No:
            CollectionOp(
                parent=cast(QWidget, reviewer.web),
                op=lambda col: act_on_card(col, card),
            ).success(lambda out: notify(action_msg(agains, passed)) if out else None).run_in_background()
